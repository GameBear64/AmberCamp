...
module.exports = [
  ...
  async ({ io, socket }, data) => {
    ...
    await group.updateOne(
      ...
      { $push: { participants: data.participants.map((p) => ({ user: ObjectId(p) })) } },
      { timestamps: false }
    );

    group = await ConversationModel.findOne({ _id: data.groupId });

    return io.to(group.participants.map(({ user }) => user.toString())).emit('group/participants/added', data.participants);
  },
];
