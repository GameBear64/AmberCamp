...
export default function Modal({children, closeFunction, easyClose, title}) {
  ...
  return createPortal(
    <div
      ...
      onClick={(e) => { if (easyClose && Object.values(e.target.classList).includes('background-div')) closeFunction() }}
    >
      <div className="w-2/3 min-w-[20em] max-w-[80vw] rounded-lg bg-base px-4 py-3 text-txtPrimary md:w-1/2 lg:max-w-[60vw] xl:w-1/3">
        <div className="flex justify-between border-b-2 border-primary pb-1">
          <p className="text-xl font-bold"> { title } </p>
          { closeFunction && <button onClick={closeFunction}> <Icon icon="close" /> </button> }
        </div>
